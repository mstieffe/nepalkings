...
We collect account data such as username, password hash, optional email address, legal acceptance timestamps, and login/session tokens. We also store gameplay data such as games, cards, figures, battle moves, spells, kingdom state, rankings, chat messages, logs, and match results.
...
Account and gameplay data are retained while needed to operate the service, preserve game history, handle disputes, or protect security. Logs may be kept for a limited operational period. Data may be removed or anonymized when no longer needed.
...
You may stop using the service at any time. For account or data questions, contact the operator listed on the public project or deployment page.
